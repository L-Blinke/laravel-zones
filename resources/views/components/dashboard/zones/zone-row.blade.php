<a href="javascript:void(0)">
    <div class="group grid place-content-center row-span-1 @if ( empty(App\Models\Call::where('type', '=', 'Blue')->where('zone_id', '=',($zone+1))->where('resolutionStatus', '=', null)->get()[0]) == false) bg-blue-400 @else {{(App\Models\Zone::find($zone+1)->nurse != null) ? : "bg-slate-600"}} {{(App\Models\Zone::find($zone+1)->nurse == null) ? : "bg-green-400"}} {{(App\Models\Zone::find($zone+1)->nurse == null && App\Models\Zone::find($zone+1)->patient == null) ? : "bg-green-600"}}
    @endif box-content h-16 w-full hover:scale-125 transition-all relative">
